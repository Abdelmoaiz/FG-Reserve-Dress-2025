...
function createExpenses() {
    if(expensesDate.value != '' && expensesType.value != '' && price.value != ''){
        ...
        if(mood == 'create'){
            ...
            ipcRenderer.send('myExpensesOthers',newExpenses)

        }else{
            ipcRenderer.send('update-expensesOthers', {...newExpenses, tmp})
        }
    }
    showExpenses();
}
...
function showExpenses() {
    ...
    ipcRenderer.on('get-expensesOthers',(e,args)=>{
        ...
        myExpenses.map(mEx=>{
            tableExpenses += `
            <tr>
                <td>${mEx.myExpensesOthers.expensesDate}</td>
                <td>${mEx.myExpensesOthers.expensesType}</td>
                <td>${mEx.myExpensesOthers.price}</td>
                <td>${mEx.myExpensesOthers.expensesDetails}</td>
...
function updateData(id) {
    ...
    for(let i=0;i<expenses[expenses.length-1].length; i++){
        if(expenses[expenses.length-1][i]._id === id){
            let allExpenses = expenses[expenses.length-1][i].myExpensesOthers;
...
function checkPass(){
    ipcRenderer.send('delete-expensesOthers',tmpIdDelete)
